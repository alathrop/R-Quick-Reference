...
From Stanford Introduction to Statistical Learning course 
Chapter 7: Beyond Linearity
R video session @ http://bit.ly/1YWox6a | Videos 5-6
...
# matlines plots both se lines (function plots columns of matrices)
...
```{r splinePlot}
require(splines)
fit=lm(wage~bs(age,knots=c(25,40,60)),data=Wage) # bs gives basis for cubic polynomials
par(mfrow=c(1,1))
plot(age,wage,col="darkgrey")
...
require(gam)
# s indicates 'smooth' function (smoothing spline)
...
# we DON'T need a nonlinear tern (i.e. s(year) since P-value of anova > 0.05)
```
...
```{r fig.width=10, fig.height=5}
par(mfrow=c(1,3))
# ns gives basis as natural apline
